...
| PL-015        | A      | Real Chromium/Firefox second-extension no-fight/recovery plus Firefox locked enterprise Proxy policy network evidence             | None beyond aggregate signed-package release gates                 |
...
| PL-096        | A      | Performance E2E plus one-minute warm-up and 5-minute Chromium heap/DOM/process-memory soak under `docs/quality/`                  | None beyond aggregate release gates                                |
...
| PL-112        | B      | Least-privilege idempotent create/refresh workflow, local failure tests and remote-asset SHA verifier; no store publisher         | Real hosted tag/release/idempotency run                            |
...
| PL-116        | B      | Safe About/Diagnostics, redaction tests and validated centralized optional resource links without placeholders                    | Final public privacy/support links                                 |
